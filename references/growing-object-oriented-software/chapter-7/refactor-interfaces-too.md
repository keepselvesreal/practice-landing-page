Line1 # Refactor Interfaces Too (pp.63-64)
Line2 
Line3 ---
Line4 **Page 63**
Line5 
...
Line50 **Page 64**
Line51 
Line52 malleable because more components will be “plug-compatible,” so we can work
Line53 at a higher level of abstraction. For the developer, there’s a secondary advantage
Line54 that there will be fewer concepts that cost time to understand.
Line55 Alternatively, if similar interfaces turn out to represent different concepts, we
Line56 can make a point of making them distinct, so that the compiler can ensure that
Line57 we only combine objects correctly. A decision to separate similar-looking inter-
Line58 faces is a good time to reconsider their naming. It’s likely that there’s a more
Line59 appropriate name for at least one of them.
Line60 Finally, another time to consider refactoring interfaces is when we start imple-
Line61 menting them. For example, if we ﬁnd that the structure of an implementing class
Line62 is unclear, perhaps it has too many responsibilities which might be a hint that
Line63 the interface is unfocused too and should be split up.
Line64 Compose Objects to Describe System Behavior
Line65 TDD at the unit level guides us to decompose our system into value types and
Line66 loosely coupled computational objects. The tests give us a good understanding
Line67 of how each object behaves and how it can be combined with others. We then
Line68 use lower-level objects as the building blocks of more capable objects; this is the
Line69 web of objects we described in Chapter 2.
Line70 In jMock, for example, we assemble a description of the expected calls for a
Line71 test in a context object called a Mockery. During a test run, the Mockery will pass
Line72 calls made to any of its mocked objects to its Expectations, each of which will
Line73 attempt to match the call. If an Expectation matches, that part of the test suc-
Line74 ceeds. If none matches, then each Expectation reports its disagreement and the
Line75 test fails. At runtime, the assembled objects look like Figure 7.1:
Line76 Figure 7.1
Line77 jMock Expectations are assembled from many objects
Line78 The advantage of this approach is that we end up with a ﬂexible application
Line79 structure built from relatively little code. It’s particularly suitable where the code
Line80 has to support many related scenarios. For each scenario, we provide a different
Line81 Chapter 7
Line82 Achieving Object-Oriented Design
Line83 64
Line84 
Line85 
Line86 ---
