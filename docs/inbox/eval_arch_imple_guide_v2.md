...
- **TrackAffiliateUseCase 포트 책임 혼재**: `TrackAffiliateUseCase`가 상태 변경 메서드(`track_click`)와 조회 메서드(`get_stats`)를 한 인터페이스에 담고 있어 CQS를 지향하는 Query Service 분리 원칙과 충돌한다 (`docs/landing_page/imple_guide_v2.md:432`). Clean Architecture는 읽기 전용 작업을 별도 Query 포트로 분리하도록 권장한다 (`references/Get_Your_Hands_Dirty_on_Clean_Architecture/4._Implementing_a_Use_Case/extracted_information.md:664`). Command/Query를 분리한 포트와 구현 클래스를 마련해 책임을 명확히 해야 한다.
- **DI 팩토리 미완성**: `get_place_order_service`가 `get_google_places_adapter()`와 `get_affiliate_persistence_adapter()`를 호출하지만 해당 팩토리 함수가 정의돼 있지 않아 의존성 조립이 완결되지 않는다 (`docs/landing_page/imple_guide_v2.md:916`). 설정 컴포넌트는 필요한 어댑터 인스턴스를 명시적으로 생성해 주입해야 한다는 장의 지침과 어긋난다 (`references/Get_Your_Hands_Dirty_on_Clean_Architecture/9._Assembling_the_Application/extracted_information.md:90`). 누락된 팩토리를 구현하거나 의존성을 주입하는 대안을 보완해야 한다.
